...
import {AddTrushName, BaseUrl, globalStyles, ShopName} from "../../../../constants";
...
export const GoodsDataScreen = ({navigation, route}) => {
    ...
    const img = Array.isArray(data.photo_list[0]) ? data.photo_list[0] : data.photo_list
    ...
    const addFavoriteFunc = async () => {
        ...
        if (!arr.is_favorite) {
            try {
                const response = await axiosInstance.post(`/favorites?good_id=${arr._id}&store_id=${arr.store_id._id}`);
                console.log(`/favorites?good_id=${arr._id}&store_id=${arr.store_id._id}`,response)
            ...
            try {
                const response = await axiosInstance.delete(`/favorites?good_id=${arr._id}`);
            ...
            <View style={styles.cont}>
                <View>
                    <LinearGradient
                        ...
                        <TouchableOpacity style={[styles.likebtn, data.is_favorite && (styles.styleLik)]}
                                          onPress={() => {
                                              addFavoriteFunc(data)
                                          }}>
                            <Image source={data.is_favorite ? like : dntLike} style={styles.likeIc}/>
                        </TouchableOpacity>
                    </LinearGradient>
                    <View style={styles.content}>
                        <Text
                            style={[globalStyles.titleText, globalStyles.textAlignLeft, globalStyles.weightLight, styles.textCont]}>
                            {data.title}
                        </Text>
                        <View style={[globalStyles.row, styles.rowCont]}>
                            <Text
                                style={[globalStyles.titleText, globalStyles.textAlignLeft, styles.textCont]}>{data.price.$numberDecimal ? data.price.$numberDecimal  : data.price} р</Text>
                            <View style={styles.rowView}>
                                <TouchableOpacity style={[styles.btnAdd,]} onPress={minNumFunc}>
                                    <Image source={minusC} style={styles.imgPlusMinus}/>
                                </TouchableOpacity>
                                <Text
                                    style={[globalStyles.titleText, globalStyles.weightBold, globalStyles.titleTextSmall, globalStyles.textAlignLeft, styles.textCount]}>{count}</Text>
                        ...
                        <View>
                            ...
                            <Text
                                style={[globalStyles.titleText, globalStyles.textAlignLeft, globalStyles.titleTextSmall4, globalStyles.weightLight, styles.idText]}>ID: {data.store_id._id}</Text>
                        </View>
                    </View>
                </View>
                <View>
                    <TouchableOpacity onPress={() => navigation.navigate(ShopName, {shop: data.store_id})}
                                      style={styles.shopContAll}>
                        <View style={styles.shopCont}>
                            <Image source={{uri: BaseUrl + '/' + data.store_id.logo_url}} style={styles.imgShop}/>
                            <View>
                                <Text
                                ...
                                <Text
                                    style={[globalStyles.titleText, globalStyles.textAlignLeft, styles.ops]}>{data.store_id.title} </Text>
                            </View>
                        </View>
                    ...
                    <View style={styles.footerCont}>
                        ...
                        <TouchableOpacity style={styles.btnView} onPress={() => {
                            ...
                            <View style={styles.priceFooter}>
                                ...
                                <Text
                                    style={[globalStyles.titleText, globalStyles.textAlignLeft, globalStyles.titleTextSmall4, globalStyles.weightLight, styles.priceText]}>{data.price.$numberDecimal ? data.price.$numberDecimal  : data.price  * count} p</Text>
                            </View>
                            <View style={styles.stylesContainer}>
                                <Text
                                    style={[globalStyles.titleText, globalStyles.titleTextSmall, globalStyles.weightBold, styles.text]}>Добавить
                                    в корзину</Text>
                            </View>
                        </TouchableOpacity>
                    </View>
