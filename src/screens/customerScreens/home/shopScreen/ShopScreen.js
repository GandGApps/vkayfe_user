...
export const ShopScreen = ({navigation, route}) => {
  ...
  const getGoods = async () => {
    try {
      const response = await axiosInstance.get(
        `/goods/by-store?store_id=${shop._id}`,
      );
  ...
  const getReview = async () => {
    try {
      const response = await axiosInstance.get(
        `/stores/reviews?store_id=${shop._id}`,
      );
    ...
    if (!item.is_favorite) {
      try {
        const response = await axiosInstance.post(
          `/favorites?good_id=${item._id}&store_id=${item.store_id}`,
        );
  ...
  const changeFav = async st => {
    ...
    if (st) {
      try {
        const response = await axiosInstance.delete(
          `/favorites/stores?store_id=${shop._id}`,
        );
            ...
            <View>
              ...
              <View style={styles.placeContainer}>
                ...
                <Text
                  ...
                  Стоимость доставки: {shop.distance.$numberDecimal} р
                </Text>
              </View>
            </View>
