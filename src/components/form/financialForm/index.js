  ...
  return (
    <TouchableOpacity style={styles.containerForm} onPress={()=> {
      ...
      <View style={styles.contentView}>
        <Text style={[globalStyles.titleText,globalStyles.weightLight,globalStyles.titleTextSmall]}>№ {item._id.substring(0,10)}</Text>
          <Text style={[globalStyles.titleText,globalStyles.weightLight,globalStyles.titleTextSmall4,
              { color: item.status_id.name === 'approved' ? "#138F2E" : "#E79800" },
          ]}>{item.status_id.title}</Text>
      </View>
      ...
      <View style={styles.contentView}>
          ...
          <Text style={[globalStyles.titleText,]}>{item.full_amount ? item.full_amount : 500} р</Text>
      </View>
    </TouchableOpacity>
  );
}
